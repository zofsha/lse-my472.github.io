...
# Fix axes 
xrange <- range(unemp$date, na.rm = TRUE)
yrange <- range(unemp$unemp, na.rm = TRUE)

p <- ggplot(unemp, aes(x = date, y = unemp)) + 
  geom_line() + 
  scale_x_date(limits = xrange, expand = c(0,0))
print(p)

# Add colored rectangles
p <- p + geom_rect(data = pres, aes(ymin = 0, ymax = 0.12, xmin= start, xmax = end, fill = party), alpha = 0.2)
print(p)

# Adding president names

geom_text(data = pres, aes(x = start, y=yrange[2]), label= name, angle = 90, hjust, vjust)

# Adding lines to separate presidencies

geom_vline(data = pres, aes(xintercept = as.numeric(start), linetype))
