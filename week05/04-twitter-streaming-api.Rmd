...
twitter_token <- create_token(app = "icedcoffeewithcream", 
...
```{r}
#stream_tweets(q = c(-125, 26, -65, 49), timeout = 30, file_name='geo_tweets.json')
#stream_tweets(q = lookup_coords("usa"), timeout = 30, file_name='__________________')

geo_tweets <- tryCatch({parse_stream('test_jsons/streamed_geo_tweets.json')},
                       error = function(e)
                           {print(paste("Retrying with alternative function after initial error when parsing file",
                                        'test_jsons/streamed_geo_tweets.json'));
                           return(recover_stream('test_jsons/streamed_geo_tweets.json'))})
```
